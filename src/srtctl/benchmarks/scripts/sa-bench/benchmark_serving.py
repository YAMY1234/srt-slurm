...
def _save_prompt_cache(cache_path: str, prompts: list[tuple[str, int, int, None]], 
                       input_len: int, output_len: int, range_ratio: float) -> None:
...
def sample_random_requests(
    ...
    if cached_prompts is not None:
        ...
        if cached_count >= num_prompts:
            ...
            print(f"[Cache] Hit! Using {num_prompts} cached prompts from {cache_path} "
                  f"(cache has {cached_count})", flush=True)
            return cached_prompts[:num_prompts]
        else:
            # Partial cache - incremental generation
            need_more = num_prompts - cached_count
            print(f"[Cache] Partial hit. Cache has {cached_count}, need {num_prompts}. "
                  f"Generating {need_more} more...", flush=True)
...
async def benchmark(
    ...
    async def print_progress(force=False):
        ...
        if force or (disable_tqdm and current_time - last_progress_time >= PROGRESS_INTERVAL):
            ...
            print(f"[Progress] Completed: {completed_count}/{total_requests}, "
    ...
    if disable_tqdm:
        print(f"[Progress] Final: Completed: {completed_count}/{total_requests}, "
              f"Failed: {failed_count}, Active: 0, Queued: 0", flush=True)
