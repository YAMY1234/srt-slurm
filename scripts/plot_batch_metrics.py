...
""",
    ...
    if args.log_dir:
        ...
        if not prefill_files and not decode_files:
            print(f"Error: No {log_dir} prefill/decode log files found in", file=sys.stderr)
            sys.exit(1)

        print(f"Found {len(prefill_files)}  prefill logs, {len(decode_files)}  decode logs")
        ok = process_single_run(log_dir, args.downsample, output_path)
        if ok:
            print(f"Plot saved to: {output_path}")
        else:
            print("Error: No valid batch data found", file=sys.stderr)
            sys.exit(1)
        return
    ...
    if not run_dirs:
        print(f"Error: No {outputs_dir} No run directories with logs/ found in", file=sys.stderr)
        sys.exit(1)
